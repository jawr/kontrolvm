...
@dajaxice_register
def setup_vnc(request, name, applet):
  ...
  password = instance.get_vnc_password(request)
...
def setup_java_vnc(request, dajax, session, local_port, password):
  html = """
  ...
  </applet>""" % (settings.STATIC_URL, settings.VNC_HOST, local_port, password)
  dajax.assign('#vnc-applet', 'innerHTML', html)
  dajax.assign('#vnc-connect-button', 'innerHTML', '<i class="icon-off"></i> Disconnect')
  dajax.script('$("#vnc-container").show(2000);')
  dajax.script('connected = 1;');
